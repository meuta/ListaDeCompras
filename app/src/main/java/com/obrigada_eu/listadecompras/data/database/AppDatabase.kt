...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
                    ...
                    c1.use {
                        ...
                        if (c1.moveToFirst()) {
                            ...
                            while (!c1.isAfterLast) {
                                cv.clear()
                                cv.put("shop_item_id", c1.getInt(c1.getColumnIndex("id")))
        ...
        private val migration11to14 = object : Migration(11, 14) {
            ...
            override fun migrate(db: SupportSQLiteDatabase) {
                try {
                    ...
                    c.use {
                        createNewShopListsTable11to14(db)
                        if (c.moveToFirst()) {
                            var position = -1
                            val cv = ContentValues()
                            while (!c.isAfterLast) {
        ...
        private val migration14to15 = object : Migration(14, 15) {
            ...
            override fun migrate(db: SupportSQLiteDatabase) {
                try {
                    ...
                    c.use {
                        createNewShopItemsTable14to15(db)
                        if (c.moveToFirst()) {
                            var position = -1
                            val cv = ContentValues()
                            while (!c.isAfterLast) {
        ...
        private fun createNewShopItemsTable14to15(db: SupportSQLiteDatabase) {
            db.execSQL(
        ...
        private fun createNewShopListsTable11to14(database: SupportSQLiteDatabase) {
            database.execSQL(
